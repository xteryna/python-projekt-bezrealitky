...
if st.session_state["zobrazit_uvodni_obrazovku"]:
    ...
    vybrany_kraj = st.sidebar.selectbox("Vyber kraj, ke kterému se bude vztahovat analýza", kraje, index=kraje.index(st.session_state.vybrany_kraj), key="vybrany_kraj")

    # funkce pro podmíněné formátování
    def zvyrazni_radek(radek):
        if radek["Kraj"] == vybrany_kraj:
